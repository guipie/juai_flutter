...
final apiServerHistoryProvider =
    StateNotifierProvider.autoDispose<ApiServerHistoryNotify, List<String>>(
        (ref) {
...
final openAiListProvider = StateNotifierProvider.family<OpenAIListNotify,
    AsyncValue<List<AllModelBean>>, APIType>((ref, apiType) {
...
class OpenAIListNotify extends StateNotifier<AsyncValue<List<AllModelBean>>> {
  ...
  Future updateModelType(int time, String modelTypeName) async {
    ...
    //查看模型是否存在
    var exist =
        openAi.supportedModels?.any((element) => element.id == modelTypeName) ??
            false;
    if (exist) {
    ...
    openAi.supportedModels
        ?.add(SupportedModels(id: modelTypeName, ownedBy: "user"));
    await HiveBox().openAIConfig.put(time.toString(), openAi);
    load();
  }

  Future<bool> add(AllModelBean openAi,
      {bool needAsync = true, bool needReload = true}) async {
    //校验别名是否重复
    if (HiveBox()
        .openAIConfig
        .values
        .toList()
        .where((element) => element.alias == openAi.alias)
        .isNotEmpty) {
    ...
    if (HiveBox()
        ...
        .values
        .toList()
        .where((element) => element.apiKey == openAi.apiKey)
        .isNotEmpty) {
  ...
  Future<bool> update(AllModelBean openAi,
      {bool needAsync = true, bool needReload = true}) async {
    await HiveBox().openAIConfig.put(openAi.time.toString(), openAi);
    if (needAsync) {}
    if (needReload) {
  ...
  Future<void> load() async {
    state = await AsyncValue.guard(() async {
      var list = HiveBox()
          .openAIConfig
          .values
          .where((element) => element.model == apiType.code)
          .toList();
      ref.watch(specialModelCountProvider(apiType).notifier).state =
          list.length;
      return list;
    });
  }
...
final specialModelCountProvider =
    StateProvider.family<int, APIType>((ref, type) {
  return HiveBox()
      .openAIConfig
      .values
      .where((element) => element.model == type.code)
      .length;
});
