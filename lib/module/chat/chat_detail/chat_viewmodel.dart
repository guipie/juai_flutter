...
final currentChatParentItemProvider =
    StateProvider.autoDispose<ChatParentItem?>((ref) {
  return null;
});
...
///语音聊天页面正在使用的模型
final currentGenerateAudioChatModelProvider =
    StateProvider<AllModelBean?>((ref) {
  return null;
});

///语音聊天中的文本解析模型
final currentGenerateAudioChatTextParserProvider =
    StateProvider<AllModelBean?>((ref) {
  return null;
});

///语音聊天页面正在使用的模型
final currentGenerateTranslateChatModelProvider =
    StateProvider<AllModelBean?>((ref) {
  return null;
});
...
final chatProvider = StateNotifierProvider.autoDispose
    .family<ChatNotify, AsyncValue<List<ChatItem>>, int>((ref, parentID) {
...
class ChatNotify extends StateNotifier<AsyncValue<List<ChatItem>>> {
  ...
  void update(ChatItem chatItem) async {
    state.value?.removeWhere((element) => element.time == chatItem.time);
    state = AsyncValue.data([...state.value ?? [], chatItem]);
    await ChatItemProvider().updateItem(chatItem);
  }
