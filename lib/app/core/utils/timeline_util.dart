...
class TimelineUtil {
  ...
  static String format(
    ...
    String timeline;
    if (info.customYesterday().isNotEmpty && DateUtil.isYesterdayByMs(ms, locTimeMs0)) {
  ...
  static String formatA(
    ...
    if (DateUtil.isWeek(ms, locMs: locTimeMs)) {
      return DateUtil.getWeekdayByMs(ms, languageCode: languageCode, short: short);
    }
  ...
  static String _getYesterday(
    ...
    return info.customYesterday() + (dayFormat == DayFormat.Full ? (' ${DateUtil.formatDateMs(ms, format: 'HH:mm')}') : '');
  }
  ...
  static String _getYear(
    ...
    if (dayFormat != DayFormat.Simple) {
      return DateUtil.formatDateMs(ms, format: (dayFormat == DayFormat.Common ? 'yyyy-MM-dd' : 'yyyy-MM-dd HH:mm'));
    }
    return '';
  }
