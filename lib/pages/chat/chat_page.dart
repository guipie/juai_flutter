...
import 'package:chat_bot/components/send_button.dart';
...
import 'package:chat_bot/module/chat/chat_detail/chat_setting_page.dart';
        ...
        return Scaffold(
                    ...
                    PullDownMenuItem(
                      ...
                      onTap: () {
                        ...
                        _screenshotController
                          ...
                          try {
                            imageFile = value;
                            final imagePath = '${(await getApplicationDocumentsDirectory()).path}/${DateTime.now().millisecondsSinceEpoch}.png';
                        ...
                        onTap: () async {
                          showCommonDialog(
                            ...
                            confirmCallback: () {
                              ...
                              ref.watch(currentChatParentItemProvider.notifier).update((state) => result.copyWith(title: S.current.new_chat));
                            },
                          );
                        }),
                  ];
          ...
          body: Consumer(builder: (context, ref, _) {
            ...
            return MultiStateWidget(
                value: chat,
                data: (list) {
                  if (requestTitled == false && list.length >= 2 && result.title == S.current.new_chat && result.id != specialGenerateTextChatParentItemTime && ref.watch(isGeneratingContentProvider) == false && ref.watch(autoGenerateTitleProvider.notifier).value == true) {
                    //list里的前2条状态必须是成功

                    requestTitled = true;
                    API().generateChatTitle(result.temperature ?? HiveBox().temperature, getModelByApiKey(result.apiKey ?? ""), result.moduleType!, list, []).then((value) {
...
class _ChatPanelState extends ConsumerState<ChatPanel> {
  ...
  void stopRecord() async {
    if (DateTime.now().millisecondsSinceEpoch - (startTime?.millisecondsSinceEpoch ?? DateTime.now().millisecondsSinceEpoch) < 1000) {
                  ...
                  Consumer(builder: (context, ref, _) {
                    return Padding(
                      ...
                      child: AnimatedCrossFade(
                        ...
                        crossFadeState: (ref.watch(sendButtonVisibleProvider) == true || disableMode == true) ? CrossFadeState.showFirst : CrossFadeState.showSecond,
                      ),
                    );
                  }),
                ],
...
List<PullDownMenuEntry> getMessageActions2(BuildContext context, WidgetRef ref, ChatItem chatItem, ResendMessage? resendMessage, SendMessageAgain? sendMessageAgain, TTSCallBack? ttsCallBack) {
  return [
...
class UserMessage extends ConsumerWidget {
  ...
  const UserMessage({super.key, required this.chatItem, required this.resendMessage, required this.sendMessageAgain, this.ttsCallBack, this.hideResend = false});
              ...
              Consumer(builder: (context, ref, _) {
                return const Icon(CupertinoIcons.exclamationmark_circle_fill, color: Colors.red, size: 20).click(() {
                  showCommonDialog(
                    ...
                    title: S.current.reminder,
                    confirmCallback: () {
                      ref.watch(chatProvider(chatItem.parentID ?? 0).notifier).remove(chatItem, connectOtherTimeID: true);
                      resendMessage(chatItem.content ?? "");
                    },
