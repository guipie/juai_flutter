...
class _OllamaAddPageState extends ConsumerState<OllamaAddPage> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(widget.openAi?.time != null ? '${S.current.edit} Ollama ${S.current.servers}' : '${S.current.btn_add} Ollama ${S.current.servers}'),
        ),
        body: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              SettingWithTitle(
                label: 'Server',
                widget: CommonTextField(maxLine: 3, color: Theme.of(context).canvasColor, controller: controller, hintText: 'http://localhost:11434'),
              ),
              const SizedBox(height: 15),
              Container(
