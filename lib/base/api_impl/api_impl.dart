...
abstract class APIImpl {
  ...
  List<RequestParams> generateChatTitleListParams(List<ChatItem> chatItems) {
    ...
    for (var j = 0; j < historyMessages.length; j++) {
      ...
      var role = requestParams.last.role;
      ...
      if (historyMessages[j].type == role) {
        ...
        requestParams.add(RequestParams(role: historyMessages[j].type ?? 0, content: [historyMessages[j].content ?? ''], images: historyMessages[j].images ?? []));
      }
    }
  ...
  List<RequestParams> generateChatHistory(List<ChatItem> chatItems, bool withoutHistoryMessage) {
    ...
    historyMessages.removeWhere((element) => element.content == null || element.content!.isEmpty || element.status != MessageStatus.success.index);
    ...
    for (var j = 0; j < historyMessages.length; j++) {
      ...
      if (historyMessages[j].type == role) {
        ...
        requestParams.add(RequestParams(role: historyMessages[j].type ?? 0, content: [historyMessages[j].content ?? ''], images: images ?? []));
      }
    }
