...
class RiverPagedBuilder<PageKeyType, ItemType> extends ConsumerStatefulWidget {
  ...
  const RiverPagedBuilder(
      {required InfiniteScrollProvider<PageKeyType, ItemType> provider, required this.pagedBuilder, required this.itemBuilder, required this.firstPageKey, this.limit = 20, this.pullToRefresh = false, this.enableInfiniteScroll = true, this.firstPageErrorIndicatorBuilder, this.firstPageProgressIndicatorBuilder, this.noItemsFoundIndicatorBuilder, this.newPageErrorIndicatorBuilder, this.newPageProgressIndicatorBuilder, this.noMoreItemsIndicatorBuilder, this.invisibleItemsThreshold, Key? key})
      : _provider = provider,
  ...
  @override
  ConsumerState<RiverPagedBuilder<PageKeyType, ItemType>> createState() => _RiverPagedBuilderState<PageKeyType, ItemType>();
}
...
class _RiverPagedBuilderState<PageKeyType, ItemType> extends ConsumerState<RiverPagedBuilder<PageKeyType, ItemType>> {
  ...
  void initState() {
    ...
    _pagingController = PagingController<PageKeyType, ItemType>(firstPageKey: widget.firstPageKey, invisibleItemsThreshold: widget.invisibleItemsThreshold);
  ...
  void _loadFromProvider(PageKeyType pageKey) {
    if (pageKey != widget.firstPageKey && !widget.enableInfiniteScroll) {
      return _updatePagingState(
        ref.read<PagedState<PageKeyType, ItemType>>(_provider).copyWith(nextPageKey: null),
      );
    }
