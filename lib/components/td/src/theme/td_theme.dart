...
class TDTheme extends StatelessWidget {
  ...
  static void setResourceBuilder(TDTDResourceBuilder delegate,
...
class TDThemeData extends ThemeExtension<TDThemeData> {
  ...
  TDThemeData copyWithTDThemeData(
    ...
    return copyWith(
        ...
        shadowMap: shadowMap,
        marginMap: marginMap,
        extraThemeData: extraThemeData) as TDThemeData;
  }
  ...
  ThemeExtension<TDThemeData> copyWith({
    ...
    var result = TDThemeData(
        ...
        refMap: _copyMap<String>(refMap, refMap as Map<String, String>?),
  ...
  static TDThemeData _emptyData(String name,
    ...
    return TDThemeData(
        name: name,
        colorMap: TDMap(
            factory: () => _defaultThemeData?.colorMap,
            refs: TDMap<String, Color>()),
        fontMap: TDMap(
            factory: () => _defaultThemeData?.fontMap,
            refs: TDMap<String, Font>()),
        radiusMap: TDMap(
            factory: () => _defaultThemeData?.radiusMap,
            refs: TDMap<String, double>()),
        fontFamilyMap: TDMap(
            factory: () => _defaultThemeData?.fontFamilyMap,
            refs: TDMap<String, FontFamily>()),
        shadowMap: TDMap(
            factory: () => _defaultThemeData?.shadowMap,
            refs: TDMap<String, List<BoxShadow>>()),
        spacerMap: TDMap(
            factory: () => _defaultThemeData?.spacerMap,
            refs: TDMap<String, double>()),
        refMap: TDMap<String, String>());
  }
    ...
    try {
      ...
      if (themeConfig.containsKey(name)) {
        ...
        colorsMap?.forEach((key, value) {
          ...
          if (color != null) {
            theme.colorMap.addAll({key: color});
          }
        });
        ...
        Log.e(
            'TTheme', 'load theme error ,not found the theme with name:$name');
        return null;
      }
    } catch (e) {
  ...
  ThemeExtension<TDThemeData> lerp(
      ThemeExtension<TDThemeData>? other, double t) {
...
class TDMap<K, V> {
  ...
  final DefaultMapFactory? factory;
  final TDMap<K, V>? refs;

  TDMap({this.factory, this.refs}) : _map = SplayTreeMap<K, V>();
